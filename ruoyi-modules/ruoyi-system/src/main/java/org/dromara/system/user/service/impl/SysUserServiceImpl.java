package org.dromara.system.user.service.impl;
...
import org.dromara.system.dept.domain.SysDept;
import org.dromara.system.dept.mapper.SysDeptMapper;
import org.dromara.system.oss.domain.vo.SysOssVo;
import org.dromara.system.oss.service.ISysOssService;
import org.dromara.system.post.domain.SysUserPost;
import org.dromara.system.post.domain.vo.SysPostVo;
import org.dromara.system.post.mapper.SysPostMapper;
import org.dromara.system.post.mapper.SysUserPostMapper;
import org.dromara.system.role.domain.SysUserRole;
import org.dromara.system.role.domain.vo.SysRoleVo;
import org.dromara.system.role.mapper.SysRoleMapper;
import org.dromara.system.role.mapper.SysUserRoleMapper;
...
public class SysUserServiceImpl implements ISysUserService, UserService {
    ...
    private Wrapper<SysUser> buildQueryWrapper(SysUserBo user) {
        ...
        if (StringUtils.isNotBlank(searchValue)) {
            wrapper.like("u.username", searchValue)
                .or()
                .like("u.nickname", searchValue)
                .or()
                .like("u.user_id", searchValue)
                .or()
                .like("u.phonenumber", searchValue)
                .or()
...
//        user.setUserId(sysUser.getUserId());
//        DataPermissionHelper.ignore(() -> insertUserRole(user, false));
        //返回用户id
    ...
    private void insertUserRole(Long userId, Long[] roleIds, boolean clear) {
        if (ArrayUtil.isNotEmpty(roleIds)) {
            ...
            if (!LoginHelper.isAdmin(userId)) {
                roleList.remove(UserConstants.ADMIN_ID);
            }
    ...
    @Cacheable(cacheNames = CacheNames.SYS_USER_AVATAR, key = "#userId")
    @Override
    public String selectAvatarById(Long userId) {
        SysUser sysUser = baseMapper.selectOne(new LambdaQueryWrapper<SysUser>()
            .select(SysUser::getAvatar).eq(SysUser::getUserId, userId));
        return ObjectUtil.isNull(sysUser) ? null : sysUser.getAvatar();
    }
     ...
     */
    @Override
    public Long countBetweenLoginDate(DateTime begin, DateTime end) {
        LambdaQueryWrapper<SysUser> query = Wrappers.lambdaQuery();
        if (ObjectUtil.isNotNull(begin) && ObjectUtil.isNotNull(end)) {
            query.between(SysUser::getLoginDate, begin, end);
        }
        return baseMapper.selectCount(query);
    }

    @Override
    public SysUserVo createUser(Long userId, String openid, String nickname, String avatar) {
        SysUserBo userBo = new SysUserBo();
        userBo.setUserId(userId);
        //app端用户
        userBo.setUserType(UserType.APP_USER.getUserType());
        //截取十位作为用户名
        userBo.setUsername(StringUtils.substring(openid, 0, 10));
        userBo.setNickname(nickname);
        userBo.setAvatar(avatar);
        //2是普通用户 3是app管理员 4是系统普通用户
        userBo.setRoleIds(new Long[]{2L});
        //查询初始密码
        String initPassword = SpringUtils.getBean(ISysConfigService.class).selectConfigByKey("sys.user.initPassword");
        String password = BCrypt.hashpw(initPassword);
        userBo.setPassword(password);
        //代理对象执行
        SpringUtils.getAopProxy(this).insertUser(userBo);
        return BeanUtil.toBean(userBo, SysUserVo.class);
    }
     ...
     */
    @Override
    public List<SysUserVo> listUserByDeptId(Long deptId) {
    ...
    @Override
    public Long countBetweenCreateTime(DateTime begin, DateTime end) {
        LambdaQueryWrapper<SysUser> query = Wrappers.lambdaQuery();
        if (ObjectUtil.isNotNull(begin) && ObjectUtil.isNotNull(end)) {
            query.between(SysUser::getCreateTime, begin, end);
        }
        return baseMapper.selectCount(query);
    }

    @Override
    public List<ChartVo> statHourNewUser() {
        return baseMapper.statHourNewUser();
    }
