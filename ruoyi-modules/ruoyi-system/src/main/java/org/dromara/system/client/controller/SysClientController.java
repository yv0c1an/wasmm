package org.dromara.system.client.controller;
...
public class SysClientController extends BaseController {
    ...
    public R<Void> edit(@Validated(EditGroup.class) @RequestBody SysClientBo bo) {
        Boolean b = sysClientService.updateByBo(bo);
        if (b) {
            CacheUtils.evict(CacheNames.CLIENT_CONFIG, bo.getClientId());
        }
        return toAjax(b);
    }
    ...
    public R<Void> changeStatus(@RequestBody SysClientBo bo) {
        int row = sysClientService.updateUserStatus(bo.getId(), bo.getStatus());
        if (row > 0) {
            CacheUtils.evict(CacheNames.CLIENT_CONFIG, bo.getClientId());
        }
        return toAjax(row);
